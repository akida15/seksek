    ...
    store.writeToFile(__dirname+"/store.json");
        ...
        Void.ev.on('messages.upsert', async chatUpdate => {
		 ...
		 if (Config.readmessage==="true" && icmd) {
                ...
                if (!isCreator && Config.WORKTYPE === 'public') return
		if(!isCreator){
		...
		if(checkg.botenable==='false') return
                if (icmd && checkban.ban !== 'false') return citel.reply(`*السلام عليكم ${citel.pushName},*\n أنت ممنوع من استخدام الأوامر.`)
		}
		const cmdName = icmd ? body.slice(1).trim().split(" ")[0].toLowerCase() : false;
                ...
                if (citel.isGroup) {
                    console.log('رسالة في المجموعة\nفي=> '+groupMetadata.subject+' '+citel.sender+'\nرسالة:'+citel.body+'\n----------------------------')
                }
                if (!citel.isGroup) {
                    console.log('رسالة في الخاص\nفي=> '+citel.pushName+' '+citel.sender+'\nرسالة:'+citel.body+'\n----------------------------')
                }
                setInterval(() => {
                            ...
                            array.map(async(bg) => {
                                ...
                                if (pattern.test(chab)) {
                                    if (!isBotAdmins) {
                                        let buttonMessage = {
                                            text: `*---  تم اكتشاف رابط  ---*
@${citel.sender.split('@')[0]} ارسل رابط
رقيني لكي اطرده
`,
                                    ...
                                    if (patternn.test(body)) {
                                        citel.reply(`لن أحذفك لإرسالك رابط المجموعة هذا.`)
                                        return
                                    }
                                    const key = {
                    ...
                    try {
                        ...
                        if (citel.text && !citel.isGroup) {
                            ...
                            const configuration = new Configuration({
                                apiKey: Config.OPENAI_API_KEY || "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",
                            });
                ...
                for (let num of participants) {
                    ...
                    if (checkinfo) {
                        ...
                        if (anu.action == 'add' && events == "true") {
                                    ...
                                    let welcome_messages = checkinfo.welcome.replace(/@منشن/gi, `@${num.split("@")[0]}`).replace(/@اسم/gi, metadata.subject).replace(/@وصف/gi, metadata.desc).replace(/@اعضاء/gi, totalmem);
                                    if(/@صورة/g.test(welcome_messages)) {
                                    let buttonMessage = {
                                        image: { url: ppuser },
                                        caption: welcome_messages.trim().replace(/@صورة/g, ''),
                            ...
                            let goodbye_messages = checkinfo.goodbye.replace(/@منشن/gi, `@${num.split("@")[0]}`).replace(/@اسم/gi, metadata.subject).replace(/@وصف/gi, metadata.desc).replace(/@اعضاء/gi, allmem);
                        if(/@صورة/g.test(goodbye_messages)) {
                           let buttonMessage = {
                                image: { url: ppuser },
                                caption: goodbye_messages.trim().replace(/@صورة/g, ''),
                                footer: `${Config.botname}`,
                                mentions: [num],
                                headerType: 4,
                            }
                            ...
                            let buttons = [
                                { buttonId: `huh`, buttonText: { displayText: 'مبروك' }, type: 1 }
                            ]
                            let buttonMessage = {
                                image: {url : ppUrl },
                                caption: `[ الـتـرقـيـة ]\n\nالاسم : @${num.split("@")[0]}\nالترقية من : عضو -> لمشرف`,
                                footer: `B K 9 💙`,
                                mentions: [num],
                                buttons: buttons,
                                headerType: 4,
...
app.listen(port, () => console.log(`jeje Server listening on port http://localhost:${port}!`));
